...
for archivo_rar in *.rar; do
    ...
    for archivo_libro in "$carpeta_destino"/*; do
        ...
        titulo_libro="$numero_ejemplar"
        nombre_libro=$(echo "$titulo_libro" | sed 's/\([^(]*\).*/\1/')
        numero_libro=$(echo "$titulo_libro" | sed 's/.* \([0-9]*\) .*/\1/')
        numero_libro=$(echo "$numero_libro" | sed 's/[^0-9]*//g')
        
        fecha_raw=$(echo "$libro" | sed -n 's/.* \([0-9]\{2\}[a-zA-Z]\+[0-9]\{4\}\).*/\1/p')
        fecha=$(date -d "$fecha_raw" +"%Y-%m-%d")


        echo $titulo_libro
        # Agregar el libro a Calibre
        #calibredb add --title "$titulo_libro" --authors "Nombre del Autor" "$archivo_libro"
        calibredb add --title "$titulo_libro" --series "$nombre_serie" --series-index "$numero_libro" --tag "Historietas, Retro Pdf" "$archivo_libro"
    done
done
